...
- A villager's trades, saved when you open its trade screen. A villager or a Wandering Trader whose trades you open keeps its offers in the download, so it trades the way the one you met did. Opening is what saves them, the rule a Chest follows, and like a Chest an animal carries they need both the Entities and Containers settings on.
...
A container you did open can go unsaved too, and it does not stop at one: on an ordinary server running an inspector plugin, a single click the plugin answered was followed by seven containers that saved nothing over the next twenty seconds. It starts with a right-click on a container the server will not open for you: a Shulker Box with no room to open, or a Chest a plugin keeps shut, whether by a lock, a claim, or an inspector tool that answers the click with a line of history instead of a screen. Your client sent the click and no screen came back, and the mod cannot tell a click that will never be answered from a screen that is merely slow to arrive, so the containers you open next go unrecorded while it waits. The missed ones keep their outline, and twenty seconds without right-clicking a container puts things right; the check and the steps are in [an outline that stays after you opened it](/guides/check-coverage/#an-outline-that-stays-after-you-opened-it).
